...
!function (a) {
  ...
  c.query = function (a) {
    ...
    b = d[f](a[0].substr(1)),
    b ? a[1] ? /^\./.test(a[1]) ? (i = a[1].substr(1), j = new RegExp('\\b' + i + '\\b'), e = [
    ], h = d.getElementsByClassName ? b.getElementsByClassName(i)  : b[g]('*'), c.each(h, function (a, b) {
  ...
  c.checkVoid = function (a, b, d) {
    ...
    a < c.mins[0] ? e = [
      ...
      'y',
      1
    ] : a >= c.mins[0] && a <= c.maxs[0] && (a == c.mins[0] && (b < c.mins[1] ? e = [
      'm'
    ] : b == c.mins[1] && d < c.mins[2] && (e = [
      'd'
    ])), a == c.maxs[0] && (b > c.maxs[1] ? e = [
      'm',
      1
    ] : b == c.maxs[1] && d > c.maxs[2] && (e = [
      'd',
      1
    ]))),
    e
  ...
  c.check = function () {
    ...
    if ('' !== d.replace(/\s/g, '')) {
      ...
      f.auto ? c.creation([e[0],
      0 | e[1],
      0 | e[2]], 1)  : f.value !== c.elem[h.elemv] && (c.elem[h.elemv] = f.value)
    }
  },
  c.months = [
    31,
  ...
  c.events = function () {
    ...
    c.each(b('#laydate_table td'), function (a, b) {
      c.on(b, 'click', function (a) {
        c.hasClass(this, h[1]) || (c.stopmp(a), c.creation([0 | this.getAttribute('y'),
  ...
  laydate.now = function (a, b) {
    var d = new Date(0 | a ? function (a) {
      return 86400000 > a ? + new Date + 86400000 * a : a
    }(parseInt(a))  : + new Date);
    return c.parse([d.getFullYear(),
    d.getMonth() + 1,
    d.getDate()], [
      d.getHours(),
      d.getMinutes(),
